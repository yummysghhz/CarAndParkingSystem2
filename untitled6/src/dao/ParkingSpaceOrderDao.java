...
public class ParkingSpaceOrderDao {
    ...
    public List<ParkingSpaceOrder> findByState(String state) throws SQLException {
        String sql = "select * from parking_space_order where state=? order by update_time desc";
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        return runner.query(sql, new BeanListHandler<>(ParkingSpaceOrder.class), state);
    }
}

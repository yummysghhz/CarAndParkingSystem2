...
public class CarOrderDao {
    ...
    public List<CarOrder> findByUserTypleAndId(String keyId, int id) throws SQLException {
        String sql = "select a.*,user.name 'tenant_name' from" +
                "( select car_order.*,user.name 'owner_name' from car_order,user where car_order.owner_id=user.id)" +
                " a, user where a.tenant_id=user.id " +
                "and a." + keyId + "=? order by a.update_time desc";
        List<CarOrder> list = null;
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        list = runner.query(sql, new BeanListHandler<>(CarOrder.class), id);
        return list;
    }
    ...
    public List<CarOrder> findByState(String state) throws SQLException {
        String sql = "select a.*,user.name 'tenant_name' from" +
                "( select car_order.*,user.name 'owner_name' from car_order,user where car_order.owner_id=user.id)" +
                " a, user where a.tenant_id=user.id " +
                "and a.state=? order by a.update_time desc";
        List<CarOrder> list = null;
        QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
        list = runner.query(sql, new BeanListHandler<>(CarOrder.class), state);
        return list;
    }
}
